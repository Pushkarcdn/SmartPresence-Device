...
const All = ({ data, status, jobs, refetch }: any) => {
              ...
              {filteredData?.map((item: any, index: number) => (
                <tr key={index}>
                  ...
                  <td className="border-t px-4 py-5 text-left text-sm text-gray-500">
                    {jobs?.find((job: any) => job?.id === item?.jobId)?.title}
                  </td>

                  <td className="border-t px-4 py-5 text-left text-sm text-gray-500 text-nowrap">
                    {
                      jobs?.find((job: any) => job?.id === item?.jobId)
                        ?.companyName
                    }
                  </td>
                  ...
                  {item?.status ? (
                    ...
                    <td className="border-t px-4 py-5 text-left text-sm text-gray-500"></td>
                  )}
